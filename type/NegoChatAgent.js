...
NegoChatAgent.prototype = {
  ...
  //next bid to offer according to the A stuck
    ...
    if (self.A.length > 0){ //if A.length is bigger than 0 it is mean that there are more issues to discuss on.
      ...
      if (goleR){
        ...
        console.log("in pickbid somewhere")
        return (goleR);
      }
      else
        return;
  ...
  opponentAccepted: function (offer, turn){
    ...
    if ((self.A.length == 0) && !finish){
      ...
      if (offerWithAccept)
      {
        if (_.keys(offerWithAccept).length == 0)
          return "done";
        else
  ...
  checkBid: function (offer, turn, lastaccept, socket){
      ...
      if (self.temp){
        ...
        if (offerWithAccept){
          if (offerWithAccept != "done"){
            ...
            if (lastaccept)
            {
                socket.emit("message", "This is great that you accept")
                return ([{'StartNewIssue': offerWithAccept[self.gole]}, {"Offer" : offerWithAccept}]); 
            }
              else
              {
                console.log("place3")
                ...
                if (_.keys(offer).length == 1)
                  ...
                  return ([{"Accept" : true}, {'StartNewIssue': offerWithAccept[self.gole]}, {"Offer" : offerWithAccept}]);
              }
            // return ([{"Accept" : offer}, {'StartNewIssue': offerWithAccept[self.gole]}, {"Offer" : offerWithAccept}]);
          }
          else{
           ...
           if (_.keys(offer).length == 1)
                ...
                return ([{"Accept" : true}, {"currentAgreement": bobj}]);
            // return ([{"Accept" : offer}, {"currentAgreement": bobj}]);
          }
        }
        else{
        ...
        if (self.temp){
          ...
          console.log("place6")
          console.log(theOriginalOffer)

          if (_.keys(theOriginalOffer).length == 1)
            return ([{"Reject" : theOriginalOffer}, {"Offer" : goleR}])
          else
  ...
  opponentRejected: function (offer, turn){
    // this stuff prepares a counter offer and very buggy
    // it comes up with issues that were agreed previously
    console.log("opponentRejected")
    console.log("offer")
    console.log(offer)
    console.log("turn")
    console.log(turn)
